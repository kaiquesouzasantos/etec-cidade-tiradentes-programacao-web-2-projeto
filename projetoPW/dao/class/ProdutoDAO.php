<?php
    class ProdutoDAO {
        ...
        public static function atualizarProduto($codigo, $nome, $preco, $desc, $quantidade, $codCategoria, $imagem){
            $produto = new Produto();
            $produto->construct_full(
                $codigo, $nome, $preco, $desc, $quantidade, CategoriaDAO::retornaCategoria($codCategoria), $imagem
            );

            try{
                $insertProduto = "CALL spAtualizaProduto(:codigo, :nome, :preco, :descr, :quantidade, :categoria, :img, @p8)";
                $stmt = self::getConexao()->prepare($insertProduto);
        ...
        public static function deletaProduto($codProduto){
            try{
                $comandoSQL = "CALL spExcluirProduto(${codProduto}, @p1)";
        ...
        public static function retornaProduto($codProduto) : Produto {
            try{
                $stmt = self::getConexao()->query("SELECT * FROM tbProduto WHERE codProduto = ${codProduto}")->fetch();
